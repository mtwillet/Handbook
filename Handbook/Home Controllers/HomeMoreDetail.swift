//
//  HomeMoreDetail.swift
//  Theatre Suite - Handbook
//
//  Created by Mathew Willett on 8/10/18.
...
class MoreDetail : UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    //Store the picked cell from home
    var mainSelection = ""
    //Store the previous picked cell
    var pastSelection = ""
    ...
    func getData(){
        SVProgressHUD.show()
        documentDownloader.downloadMoreDetail(mainDocID: mainSelection, detailDocID: pastSelection) { (documnet: [document], detailPage: [Int], dbid: [Int]) in
            for doc in documnet {
                self.detailNames.append(doc.documentID)
            }
            self.detailScreenPage = detailPage
            self.dbid = dbid
    ...
    func favoriteContextualAction(forRowAtIndexPath indexPath : IndexPath) -> UIContextualAction {
        let action = UIContextualAction(style: .normal, title: "Favorite") { (contextAction: UIContextualAction, sourceView: UIView, completionHandler: (Bool) -> Void) in
            ...
            let entity = NSEntityDescription.entity(forEntityName: "CDFavorites", in: context)
            let newFavorite = NSManagedObject(entity: entity!, insertInto: context)
            
            newFavorite.setValue(self.detailNames[indexPath.row], forKey: "documentID")
            newFavorite.setValue(self.dbid[indexPath.row], forKey: "dbid")
