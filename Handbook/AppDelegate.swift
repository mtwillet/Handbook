...
//  Handbook
//
//  Created by Mathew Willett on 8/13/18.
...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        //MARK: Set up initial view controllers
        let defaults = UserDefaults.standard
        if let launchview = defaults.string(forKey: "InitialVC") {
            if launchview == "LogInVC" {
            ...
            } else if launchview == "MainVC" {
                window = UIWindow(frame: UIScreen.main.bounds)
                let mainStoryboard: UIStoryboard = UIStoryboard(name: "Main", bundle: nil)
                let yourVC = mainStoryboard.instantiateViewController(withIdentifier: "MainVC") as! ViewController
                window?.rootViewController = yourVC
                window?.makeKeyAndVisible()
                //print("Main VC")
            }
    ...
    lazy var persistentContainer: NSPersistentContainer = {
         ...
         */
        let container = NSPersistentContainer(name: "Handbook")
